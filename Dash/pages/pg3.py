...
card_table_pred_hw = dbc.Card(
    [
        ...
        dbc.CardBody(
            [
                dash_table.DataTable(id = "data_table_pred_hw", page_size = 12, style_table = {"overflowX": "auto"}),
                html.Div(id = "output_hw")
            ]
        )
    ], className = "card border-primary mb-3"
)
...
layout = dbc.Container(
    [
        dbc.Row([
                ...
                dcc.Dropdown(id = "drop_days_hw", options = [1, 3, 5, 10, 30], value = 10, style = {"color": "black"}),
                html.P()
        ]),
        dbc.Row([
            dbc.Col(
        ...
        dcc.Store(id = "forecast_store_hw"),
        dcc.Store(id = "forecast_fit_store_hw")
    ], fluid = True
)
...
def update_Store_Data_hw(data):
    ...
    forecast_df, fit_values = HoltWinter(hist_data, 30)
